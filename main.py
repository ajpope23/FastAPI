...
def send_email(subject, message, to_address):
    ...
    msg = MIMEMultipart()
    msg['From'] = "BlockWall Street - Email verification <" + from_address + ">"
    msg['To'] = to_address
    msg['Subject'] = subject
...
async def create_lead(lead: LeadSchema):
    ...
    msg = f'<p>Welcome to BlockWall Street, {lead.name}!</p><p>Please click on the following link to verify your email:</p><a href="{email_base_url}/verify_client?token={token}&email={quote(lead.email)}&phone={quote(lead.phone)}&db_type=leads">Verify Email</a><p>Thank you,</p><p>SmartBids.ai Team</p>'
...
async def send_verification(email: EmailSchema):
    ...
    # [Rest of your email generation and sending logic]
    msg = f'<p>Welcome to BlockWall Street!</p><p>Please click on the following link to verify your email:</p><a href="{email_base_url}/verify_client?token={token}&email={quote(email.email)}&db_type=users">Verify Email</a><p>Thank you,</p><p>SmartBids.ai Team</p>'
    subject = 'Email verification'
